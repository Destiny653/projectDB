...
export function ValidatorMail() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/validMail`, {
        method: 'POST',
  ...
  return (
    <div className={`${styles.validatorParent}`}>
      <button className='top-[40px] left-[40px] absolute bg-[#000] px-[12px] py-[6px] rounded-[5px] text-[#fff]' onClick={() => { setValidMail(true) }} >Verify Email</button>
...
export function ValidatorOTP() {
  ...
  return (
    <div>
      <button className='top-[40px] left-[40px] absolute bg-[#000] px-[12px] py-[6px] rounded-[5px] text-[#fff]' onClick={() => { setValidMail(false) }} >Verify OTP</button>
      <div className={`${styles.validEmail} ${validMail && styles.setMail}`}>
        <form action="" className='flex flex-col justify-center items-center gap-[10px]' onSubmit={handleSubmit}>
          <label htmlFor="email" className='flex flex-col gap-[10px] w-full'>
            <span>OTP</span>
